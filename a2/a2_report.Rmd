...
qlf.pos_vs_neg <- glmQLFTest(fit, coef='samples$cell_typeTreatment')
...
For the upregulated genes, pathways related to mitosis (chromosome segregation, spindle formation etc.) were highly enriched (Figure 4), which is also the top items in the original study (E2F targets, G2M checkpoint, mitotic spindle). E2F targets include many DNA replication genes, which are often upregulated in cancer cells. Several DNA repair pathways also are included, which is another hallmark of a cancer cell.

![Figure 4. g:Profiler results with upregulated genes](up.png)

### Downregulated genes

For the downregulated genes, there is a wider range of pathways (Figure 5). It is unclear whether there is a clear overlap between the pathways found in this work and the original. In the original study, the most dominant term was ANDROGEN RESPONSE, which is the target pathway of CCS1477.
...
Indeed, 29 androgen response genes are in the list of downregulated genes we obtained.
This includes genes like KLK2, KLK3 and TMPRSS2, which were shown to be downregulated in-vivo using patient-derived models of lethal prostate cancer.
...
The GO terms we obtained for upregulated genes corresponded well to the original study, which included pathways that are upregulated in cancer tissues, like mitosis pathways and DNA repair pathways.

KLK2, KLK3 and TMPRSS2 were in the list of downregulated genes we obtained. which is in agreement with the original study. The GO terms we obtained for downregulated genes did not perfectly overlap with the original. This may be because a wider range of pathways are involved, parameters need to be fine-tuned to obtain the exact same result. It's also possible that using the MolSigDB as in the original study instead of g:Profiler would help in faithfully replicating the result.

# Conclusion & Outlook

The heatmap we obtained shows that some genes are significantly upregulated or downregulated as a result of CCS147 treatment. The list of GO terms obtained from g:Profiler also was in agreement with out expectations. In the next assignment, it will be interesting to look in depth at the pathways with differential expression in treatment samples.
